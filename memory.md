...
### 2024-09-30 - v1.2.1 CLI 路径检测增强 🔧
- **用户反馈驱动**：快速响应 GitHub Issue 用户反馈
  - 问题：用户已安装 Claude CLI，但快捷方式检测显示未安装
  - 原因：PATH 配置方式多样，原有检测逻辑覆盖不全面

- **扩展搜索路径**：支持更多 Node.js 和 CLI 工具的安装方式
  - 新增 `$HOME/.npm/bin` - npm 默认全局安装路径
  - 新增 `$HOME/.local/bin` - 用户本地二进制路径
  - 新增 `/usr/local/share/npm/bin` - npm 可选安装路径

- **智能路径查找**：自动遍历常见安装位置
  - 即使 PATH 未正确配置也能找到 claude 命令
  - 支持多种包管理器的安装方式（npm、brew 等）
  - 降低用户配置门槛

- **友好错误提示**：检测失败时提供详细诊断信息
  - 显示所有常见安装位置列表
  - 提供诊断命令：`command -v claude`
  - 帮助用户快速定位问题

- **重要经验教训**：首次在 Release 中上传编译好的 App
  - 之前 v1.0.0 - v1.2.0 只有源码，用户体验不好
  - v1.2.1 开始提供 `Claude-Code-Now-v1.2.1.zip`
  - 用户下载解压即可直接使用，真正做到"开箱即用"

### 2024-09-30 - Claude Code 2.0 完美适配
- **兼容性升级**：完全适配 Claude Code 2.0
  - 升级启动命令：`claude code now` → `claude`
  - Claude Code 2.0 简化了命令结构，更加简洁

- **智能路径修复**：程序坞启动不再跳转到桌面
  - 修复 AppleScript 路径回退逻辑
  - 无法获取 Finder 路径时，使用默认工作目录
